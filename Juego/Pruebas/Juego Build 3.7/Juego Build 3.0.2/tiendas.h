...
// para evitar tener que poner delante std::
using namespace std;
struct objetoventa{
    int nid;
    int id;
...
//se crea con la omagen,nombre,precio
...
void pinta_tienda()
{
     if (muestra_tienda == true )
     {
          ...
          comprar.crear("Comprar",(FONT *)datos_del_juego[dftextos].dat );
          vender.crear("Vender",(FONT *)datos_del_juego[dftextos].dat );
          salir.crear("Salir",(FONT *)datos_del_juego[dftextos].dat );
          descripcion.crear("Descripcion",(FONT *)datos_del_juego[dftextos].dat );
          precios.crear("Precio",(FONT *)datos_del_juego[dftextos].dat );
    ...
    textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+305, y_tienda+374, 0xFFFFFF, -1, "Dinero %9d ",
                   jugador.getdinero() );
   ...
   if ( tienda_estado == 1 )
   {
          ...
          for ( int it=inicia_objeto; it < vlista_tienda.size() && it < inicia_objeto+10; it++ )
          {
              ...
              masked_blit( (BITMAP*)datobjeto[vlista_tienda[it].id].dat, buffer,
                           0,0,x_tienda,ty, 32,32);

              textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+34, ty, 0xFFFFFF, -1, "%s", vlista_tienda[it].nombre );

              textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+367, ty, 0xFFFFFF, -1, "%8d", vlista_tienda[it].precio );
                     ...
                     mouse_y > y_tienda+32+(numi*34) && mouse_y < y_tienda+64+(numi*34))
                {
                     ...
                     tdesc.pinta(buffer);
                     textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+274, y_tienda, 0xFFFFFF, -1,
 ...
 if ( tienda_estado == 2 )
 {
      ...
      if ( obj_tienes == 0 )
      {
           textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat,
      ...
      if ( obj_tienes > 10 )
      {
          ...
          for ( int i=0; i < 12; i++ )
          {
                   ...
                   it < 10+inicia_objeto && it >= inicia_objeto)
              {
                   ...
                   masked_blit( (BITMAP*)datobjeto[id_img_objeto(mid)].dat, buffer,
                                0,0,x_tienda,ty, 32,32);

              textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+34, ty, 0xFFFFFF, -1, "%s", nombre_objeto(mid) );

              textprintf_ex( buffer, (FONT *)datos_del_juego[dftextos].dat, x_tienda+367, ty, 0xFFFFFF, -1, "%8d", precio_objeto(mid) );
